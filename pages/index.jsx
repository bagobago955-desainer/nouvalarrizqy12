    ...
    <div className="min-h-screen transition-colors duration-500" style={{ background: bg }}>
            ...
            <section className="max-w-5xl mx-auto text-center">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {prestasiList.map((p, i) => (
                  <Card key={i} className="hover:shadow-lg transition cursor-pointer" onClick={() => setSelectedImageIndex(karyaList.length + i)}>
                    <motion.img whileHover={{ scale: 1.03 }} src={p.img} alt={p.title} className="w-full h-40 object-cover rounded-t-lg" />
                    <CardContent className="p-4">
      ...
      <motion.div
        ...
        <motion.img
          ...
          // Tambahan efek zoom halus saat diklik
          whileTap={{ scale: 1.15 }}
